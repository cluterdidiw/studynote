...
def plot_panorama_graph(target_nodes, upstream_hierarchy, downstream_hierarchy, 
    ...
    for node in node_coords:
        ...
        node_text.append(
            f"节点：{node}<br>类型：{info['final_depth_type']}{info['final_depth'] if info['final_depth']>0 else ''}<br>关联Project：{', '.join(info['related_project']) if info['related_project'] else '无'}"
        )
